...
export default function MyIdeasPage() {
  ...
  const fetchMyIdeas = () => {
    if (!userEmail) return;
    setLoading(true);
    fetch(`https://assignment-nine-mu-five.vercel.app/api/my-ideas?email=${userEmail}`)
      .then((res) => res.json())
      .then((data) => {
        setMyIdeas(data);
        setLoading(false);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`https://assignment-nine-mu-five.vercel.app/api/ideas/${id}`, {
            method: "DELETE",
          });
  ...
  const handleUpdate = async (idea) => {
    ...
    if (newTitle) {
      try {
        const response = await fetch(`https://assignment-nine-mu-five.vercel.app/api/ideas/${idea._id}`, {
          method: "PUT",
