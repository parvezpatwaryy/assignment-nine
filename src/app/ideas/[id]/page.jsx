...
export default function IdeaDetailsPage() {
  ...
  useEffect(() => {
    ...
    const fetchDetailsData = async () => {
      try {
        setLoading(true);
        const ideaRes = await fetch(`https://assignment-nine-mu-five.vercel.app/api/ideas/${id}`);
        if (ideaRes.ok) {
        ...
        const commentsRes = await fetch(`https://assignment-nine-mu-five.vercel.app/api/comments?ideaId=${id}`);
        if (commentsRes.ok) {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://assignment-nine-mu-five.vercel.app/api/comments", {
        method: "POST",
